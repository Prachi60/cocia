...
const ProductCard = React.memo(({ product, onPress }) => (
  <div onClick={() => onPress(product)} className="flex flex-col cursor-pointer group active:scale-95 transition-transform">
    <div className="relative aspect-square rounded-xl overflow-hidden bg-gray-50 border border-gray-100">
      ...
      <div className="absolute top-2 right-2 bg-[#2874F0] text-white text-[9px] font-black px-1.5 py-0.5 rounded-full">{product.discount}</div>
    </div>
  ...
  return (
    <div className="px-3 py-2">
      ...
      <div className="flex justify-center gap-1.5 mt-2">
        {banners.map((_, i) => (
          <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${i === idx ? 'w-4 bg-[#2874F0]' : 'w-1.5 bg-gray-300'}`} />
        ))}
      </div>
    </div>
  );
    ...
    <div className="bg-white min-h-screen pb-24" style={{ fontFamily: "'Inter', Arial, sans-serif" }}>
      ...
      <div className="px-3 mt-2">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => navigate('/vendor/all-offers')} className="flex items-center gap-0.5 text-[12px] font-bold text-[#2874F0]">
          ...
          {deals.map((d, i) => (
            ...
            >
              ...
              <div>
                ...
                <span className="inline-block mt-1 bg-[#2874F0] text-white text-[9.5px] font-black px-2 py-0.5 rounded-full">{d.badge}</span>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      {/* ── Promo banner ── */}
      <div className="px-3 mt-5">
        <div className="bg-gradient-to-r from-[#1259c3] to-[#2874F0] rounded-2xl p-4 flex items-center justify-between overflow-hidden relative">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-5 py-3 flex items-center justify-between z-50 shadow-[0_-2px_10px_rgba(0,0,0,0.06)]">
        ...
        <button
          ...
          className="bg-[#2874F0] text-white px-7 py-3 rounded-xl font-black text-[13px] uppercase tracking-wider shadow-md shadow-blue-200 active:scale-95 transition-transform"
        >
          Go to Cart
        </button>
      </div>
