          ...
          <div className="w-[90px] bg-[#f8f9fb] border-r border-gray-100 overflow-y-auto no-scrollbar h-[calc(100vh-60px)]">
            {categories.map((cat) => (
              <div 
                ...
                className={`py-4 px-2 flex flex-col items-center gap-2 cursor-pointer transition-all relative ${
                  activeCategory === cat ? 'bg-white' : 'bg-transparent'
                }`}
              >
                {activeCategory === cat && (
                ...
                <span className={`text-[10px] text-center leading-tight font-bold transition-colors ${
                  activeCategory === cat ? 'text-[#2874f0]' : 'text-slate-500'
                }`}>
                  {cat}
                </span>
              </div>
